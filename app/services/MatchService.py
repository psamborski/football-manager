...
from .utils import SCORING_MODIFIERS


class MatchService:
    def __init__(self, hosts_club: ClubService, guests_club: ClubService):
        self.host_team = hosts_club
        self.guest_team = guests_club
        self.score = {hosts_club.club_id: 0, guests_club.club_id: 0}
        self.match_time = 90
        self.stats = {
    ...
    def _attempt_goal(self, team, current_time, segment_duration):
        ...
        probability = team.club_rating["first_eleven_strength"] / (
                    self.host_team.club_rating["first_eleven_strength"] + self.guest_team.club_rating["first_eleven_strength"])
        ...
        if randomizer < probability:
            ...
            self.stats[team.club_id]["scorers"].append(
                [self._pick_scorer(team.players_models), self._pick_goal_time(current_time, segment_duration)]
            )
            return True
        else:
            return False
    ...
    def _pick_scorer(players):
        ...
        position_points = SCORING_MODIFIERS
